...
public class NumberToStringConvertor {
    ...
    private static String getStringForHundreds(int n) {
    ...
    private static String getStringForTens(int n) {
        StringBuilder numberAsString = new StringBuilder();

        if (n / 10 != 1) {
            return getStringForHigherTens(n);
        } else {
            return getStringForLowerTens(n);
        }
    }
    ...
    private static boolean doesConsistHundreds(int number) {
        return number / 100 != 0;
    }

    private static boolean doesConsistTens(int number) {
        return number / 10 != 0;
    }
}
